...
if (!class_exists('SR_Typography')) {
	...
	class SR_Typography {
		...
		public function get_fonts($font = null, $refresh = false) {
			...
			$fonts = get_option('sr_ots_available_fonts' , $default = false);
			...
			if ($fonts === false || $refresh === true ){
				 ...
				 * Custom Fonts
				 */
				$custom_fonts = offtheshelf_option('web_fonts_custom', false);
				...
				if(! is_wp_error( $gs ) ) {
					...
					if (!isset($fontArray->items)) {
						$fontArray = json_decode(offtheshelf_file_read_contents( get_template_directory() . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'googlefonts.json' ));
					}
					if (isset($fontArray->items)) {
				...
				update_option( 'sr_ots_available_fonts' ,$fonts);
			}
		...
		public function get_fonts_in_use($font_options) {
			...
			foreach ($font_options as $font_option) {
				$font_option = offtheshelf_option( $font_option );
		...
		public function get_font_family( $option ) {
			$option = offtheshelf_option ( $option, false );
