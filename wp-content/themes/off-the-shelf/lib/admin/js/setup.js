jQuery(document).ready(function($) {
	var offtheshelf_setup_is_error = false;
	...
	$('#start-template-setup').click(function(e) {
		...
		offtheshelf_setup_ajax ( 'init', folder, 'Preparing import', 0 );
	});


	function offtheshelf_setup_error( message ) {
		offtheshelf_setup_is_error = true;
	...
	function offtheshelf_setup_ajax( action, folder, status_message, last ) {
		...
		$.ajax({
			...
			data: {
				...
				action : "offtheshelf_setup_" + action,
				last: last,
				folder: folder,
			...
			success: function(response) {
				...
				if(response.type == "success") {
					...
					if (!action.length) {
						offtheshelf_setup_error( 'Unexpected response from server.' );
					}
					...
					if (action == 'init' )
						if ( ! skip_images ) {
							...
							offtheshelf_setup_ajax ( 'images', folder, 'Importing attached images (' + last + ')', last );

						} else {
							// we don't want images imported, so skip to the next step
							offtheshelf_setup_ajax ( 'profiles', folder, 'Skipping Images. Importing profiles', 0 );
						}
					if (action == 'images' )
						offtheshelf_setup_ajax ('profiles', folder, 'Importing profiles', 0 );
					if (action == 'profiles' )
						offtheshelf_setup_ajax ('modals', folder, 'Importing modals', 0 );
					if (action == 'modals' )
						offtheshelf_setup_ajax ('banners', folder, 'Importing banners', 0 );
					if (action == 'banners')
						offtheshelf_setup_ajax ('pages', folder, 'Importing pages', 0 );
					if (action == 'pages')
						offtheshelf_setup_ajax ('finalize', folder, 'Finalizing import', 0 );
					if (action == 'finalize') {
					...
					_ajax_nonce = response.newNonce;
					$('#_wpnonce').val( _ajax_nonce );
					offtheshelf_setup_error( response.message );
				}
			},
			error: function(e, x, settings, exception) {
				...
				if (x.status) {
					...
					offtheshelf_setup_error( errorMessage );
					console.log("Error message is: " + errorMessage);
				} else {
					offtheshelf_setup_error( 'Unexpected error. ' + e.responseText );
