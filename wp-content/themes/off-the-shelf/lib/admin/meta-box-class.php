...
if ( ! class_exists( 'SR_Meta_Box' ) ) :
	class SR_Meta_Box {
		...
		public function add_typography_data() {
			...
			echo json_encode( $fonts_export ) . ";\n" . 'var font_picker_select = "' . esc_attr( esc_html__( 'Update', 'offtheshelf' ) ) . '";' . "</script>\n";
		}
		...
		public function show_field_icon( $field, $meta ) {
			...
			echo '<input type="button" class="button fontawesome-picker" data-target="#' . esc_attr( $id_sanitized ) . '" id="' . 'picker_' . esc_attr( $id_sanitized ) . '" value="' . esc_html__( 'Select Icon', 'offtheshelf' ) . '">';
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_image( $field, $meta ) {
			...
			echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
			if ( $has_image ) {
				echo "<input class='{$multiple} button simplePanelimageUploadclear' id='{$id}' value='" . esc_html__( 'Remove Image', 'offtheshelf' ) . "' type='button'/>";
			} else {
				echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='" . esc_html__( 'Upload Image', 'offtheshelf' ) . "' type='button'/>";
			}
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_gallery( $field, $meta ) {
			...
			$out .= '<input type="button" class="button gallery-picker-select" data-target="#' . $name . '" id="' . $name . '_ids' . '" value="' . esc_html__( 'Select Items', 'offtheshelf' ) . '">';
		...
		function Validate_upload_file_type( $file ) {
			if ( isset( $_POST['uploadeType'] ) && ! empty( $_POST['uploadeType'] ) && isset( $_POST['uploadeType'] ) && $_POST['uploadeType'] == 'my_meta_box' ) {
				...
				if ( ! in_array( $ext, (array) $allowed ) ) {
					$file['error'] = esc_html__( "Sorry, you cannot upload this file type for this field.", 'offtheshelf' );
