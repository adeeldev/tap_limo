...
if ( ! class_exists( 'SR_Tax_Meta' ) ) :
     ...
     */
        ...
        <img src="';
					...
					echo '" alt="' . esc_html__( 'Remove', 'offtheshelf' ) . '" title="' . esc_html__( 'Remove', 'offtheshelf' ) . '" id="remove-' . $field['id'] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . esc_html__( 'Add', 'offtheshelf' ) . '" title="' . esc_html__( 'Add', 'offtheshelf' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_begin( $field, $meta ) {
			...
			if ( $field['name'] != '' || $field['name'] != false ) {
				...
				echo "<label for='{$field['id']}'>" . esc_html($field['name']) . "</label>";
				//echo "</div>";
			}
			if ( $this->_form_type == 'edit' ) {
				echo '</th><td>';
			...
			if ( isset( $field['group'] ) ) {
				if ( $group == 'end' ) {
					if ( isset( $field['desc'] ) && $field['desc'] != '' ) {
						echo "<p class='description'>" . esc_html($field['desc']) . "</p></td>";
					} else {
						echo "</td>";
					}
				} else {
					if ( isset( $field['desc'] ) && $field['desc'] != '' ) {
						echo "<p class='description'>" . esc_html($field['desc']) . "</p><br/>";
					} else {
						echo '<br/>';
					}
				}
