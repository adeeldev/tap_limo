...
if ( ! class_exists( 'SR_Admin_Page' ) ) :
     ...
     */
		...
		public function edit_insert_to_post_text( $safe_text, $text ) {
			if ( is_admin() && 'Insert into Post' == $safe_text ) {
				if ( isset( $_REQUEST['apc'] ) && 'insert_file' == $_REQUEST['apc'] ) {
					return str_replace( esc_html__( 'Insert into Post', 'offtheshelf' ), esc_html__( 'Use this File', 'offtheshelf' ), $safe_text );
				} else {
					return str_replace( esc_html__( 'Insert into Post', 'offtheshelf' ), esc_html__( 'Use this Image', 'offtheshelf' ), $safe_text );
				}
			}
		...
		public function DisplayPage() {
			...
			echo '<h2>' . esc_html__( 'Off the Shelf Options', 'offtheshelf' );

			if ( defined ('OFFTHESHELF_FEATURE_PACK') ) {
				echo '<a href="' . esc_url( admin_url( 'edit.php?post_type=profile' ) ) . '" id="sr-profiles" class="add-new-h2">' . __( 'Profiles', 'offtheshelf' ) . '</a>';
				echo '<a href="' . esc_url( admin_url( 'edit.php?post_type=banner' ) ) . '" id="sr-banners" class="add-new-h2">' . __( 'Banners', 'offtheshelf' ) . '</a>';
			}

			if ( function_exists( 'siteorigin_panels_render' ) ) {
				echo '<a href="' . esc_url ( admin_url( 'themes.php?page=offtheshelf-setup&from=options' ) ) . '" id="sr-banners" class="add-new-h2">' . __( 'Templates', 'offtheshelf' ) . '</a>';
			}

			if ( ! offtheshelf_hide_support_links() ) {
				echo '<a href="' . esc_url ( SR_SUPPORT_URL ) . '" class="add-new-h2" id="sr-support" target="_blank">' . __( 'Customer Support', 'offtheshelf' ) . '</a>';
			}
			...
			echo '<div class="header_wrap">
              <input type="submit" value="' . esc_attr( __( 'Save Changes', 'offtheshelf' ) ) . '" name="Submit" class="' . apply_filters( 'admin_page_class_submit_class', 'button button-primary menu-save admin-options-save' ) . ' btn">
      		  </div>';
				...
				if ( method_exists( $this, 'show_field_' . $field['type'] ) ) {
					...
					switch ( $field['type'] ) {
						...
						case 'TABS_Listing':
							...
							if ( $this->saved_flag ) {
								...
								if ( is_array( $this->errors ) && count( $this->errors ) > 0 ) {
									...
									echo '<p>' . __( 'Settings saved.', 'offtheshelf' ) . '</p>';
								}
								echo '</div>';
							}
						...
						case 'subtitle':
							...
							break;
						case 'paragraph':
							echo '<p>' . offtheshelf_esc_html ( $field['text'] ) . '</p>';
							break;
						case 'repeater':
        ...
        <div style="apc_submit_footer">
          <input type="submit" name="Submit" class="' . apply_filters( 'admin_page_class_submit_class', 'button button-primary menu-save admin-options-save' ) . ' btn" value="' . esc_attr( __( 'Save Changes', 'offtheshelf' ) ) . '" />
        </div>
      </div>';
		...
		public function load_scripts_styles() {
			...
			wp_enqueue_script( 'offtheshelf-frosty', $plugin_path . '/js/frosty.min.js', array( 'jquery' ), null, true );
			wp_enqueue_script( 'offtheshelf-image-picker', $plugin_path . '/js/image-picker.min.js', array( 'jquery' ), null, true );
		...
		public function add_typography_data() {
			...
			echo json_encode( $fonts_export ) . ";\n" . 'var font_picker_select = "' . esc_attr( __( 'Update', 'offtheshelf' ) ) . '";' . "</script>\n";
		}
		...
		public function insert_images() {
			...
			foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
				...
				$li .= "<a title='" . esc_attr__( 'Delete this image', 'offtheshelf' ) . "' class='at-delete-file' href='#' rel='{$rel}'><img src='" . esc_url( $this->SelfPath . "/images/delete-16.png" ) . "' alt='" . esc_html__( 'Delete', 'offtheshelf' ) . "' /></a>";
		...
		public function show_field_repeater( $field, $meta ) {
			...
			if ( count( $meta ) > 0 && is_array( $meta ) ) {
				foreach ( $meta as $me ) {
					...
					echo '<a class="button button-warning at-icon at-re-remove" id="remove-' . $field['id'] . '" title="' . esc_attr__( 'Delete', 'offtheshelf' ) . '">' . esc_html__( 'Delete', 'offtheshelf' ) . '</a>';
					...
					echo '</div>';
					echo '<a class="button at-icon at-re-toggle" title="' . esc_attr__( 'Edit', 'offtheshelf' ) . '">' . esc_html__( 'Edit', 'offtheshelf' ) . '</a>';
					echo '</div>';
					$c = $c + 1;
			...
			echo '<a class="button at-icon at-re-add" id="add-' . $jsid . '" title="' . esc_attr__( 'Add', 'offtheshelf' ) . '">' . esc_html__( 'Add', 'offtheshelf' ) . '</a></div>';

			//create all fields once more for js function and catch with object buffer
			ob_start();

			echo '<div class="at-repeater-block at-repeater-new-item">';
			echo '<span class="at-repeater-block-title">' . esc_html__( 'New Item', 'offtheshelf' ) . '</span>';
			echo '<a class="button button-warning at-icon at-re-remove" id="remove-' . $jsid . '" title="' . esc_attr__( 'Delete', 'offtheshelf' ) . '">' . esc_html__( 'Delete', 'offtheshelf' ) . '</a>';
			...
			echo '<a class="button at-icon at-re-toggle" id="edit-' . $jsid . '" title="' . esc_attr__( 'Edit', 'offtheshelf' ) . '">' . esc_html__( 'Edit', 'offtheshelf' ) . '</a>';
			echo '</div>';
		...
		public function show_field_textarea( $field, $meta ) {
			...
			if ( isset( $field['form'] ) ) {
				echo '<a class="button-secondary clean_form" href="javascript:void(0);">' . esc_html__( 'Clean Form Code', 'offtheshelf' ) . '</a>';
			}
		...
		public function show_field_file( $field, $meta ) {
			...
			if ( ! empty( $meta ) ) {
				$nonce = wp_create_nonce( 'at_ajax_delete' );
				echo '<div style="margin-bottom: 10px"><strong>' . esc_html__( 'Uploaded files', 'offtheshelf' ) . '</strong></div>';
				echo '<ol class="at-upload">';
				foreach ( $meta as $att ) {
					// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
					echo "<li>" . wp_get_attachment_link( $att, '', false, false, ' ' ) . " (<a class='at-delete-file' href='#' rel='{$nonce}|{$post->ID}|{$field['id']}|{$att}'>" . esc_html__( 'Delete', 'offtheshelf' ) . "</a>)</li>";
				}
				echo '</ol>';
			}
			...
			echo "<strong>" . esc_html__( 'Upload new files', 'offtheshelf' ) . "</strong>";
			echo "</div>";
			echo "<div class='new-files'>";
			echo "<div class='file-input'>";
			echo "<input type='file' name='{$field['id']}[]' />";
			echo "</div><!-- End .file-input -->";
			echo "<a class='at-add-file button' href='#'>" . esc_html__( 'Add more files', 'offtheshelf' ) . "</a>";
		...
		public function show_field_media_manager( $field, $meta ) {
			...
			if ( is_array( $meta ) && isset( $meta['src'] ) && $meta['src'] != '' ) {
				...
				$html .= "<input class='at-delete_image_button button' type='button' rel='" . $field['id'] . "' value='" . esc_html__( 'Delete Image', 'offtheshelf' ) . "' />";
			} else {
				...
				$html .= "<input class='at-mm-upload_image_button button' type='button' rel='" . $field['id'] . "' value='" . esc_html__( 'Upload Image', 'offtheshelf' ) . "' />";
			}
			echo $html;
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_image( $field, $meta ) {
			...
			if ( is_array( $meta ) && isset( $meta['src'] ) && $meta['src'] != '' ) {
				...
				$html .= "<input type='hidden' name='" . $field['id'] . "[id]' id='" . $field['id'] . "[id]' value='" . $meta['id'] . "' />";
				$html .= "<input type='hidden' name='" . $field['id'] . "[src]' id='" . $field['id'] . "[src]' value='" . $meta['src'] . "' />";
				$html .= "<input class='at-delete_image_button button' type='button' data-u='" . $upload_type . "' rel='" . $field['id'] . "' value='" . esc_html__( 'Delete Image', 'offtheshelf' ) . "' />";
			} else {
				$html .= "<span class='mupload_img_holder'  data-wi='" . $width . "' data-he='" . $height . "'></span>";
				$html .= "<input type='hidden' name='" . $field['id'] . "[id]' id='" . $field['id'] . "[id]' value='' />";
				$html .= "<input type='hidden' name='" . $field['id'] . "[src]' id='" . $field['id'] . "[src]' value='' />";
				$html .= "<input class='at-upload_image_button button' type='button' data-u='" . $upload_type . "' rel='" . $field['id'] . "' value='" . esc_html__( 'Upload Image', 'offtheshelf' ) . "' />";
			}
			echo $html;
			$this->show_field_end( $field, $meta );
		}
		...
		public function show_field_button( $field, $meta ) {
			...
			$html .= '<div id="' . $field['id'] . '_status" style="display: none;"><span>' . esc_html__( 'Refreshing fonts database...', 'offtheshelf' ) . '</span></div>';
		...
		public function show_field_typography( $field, $meta ) {
			...
			$html .= '<input id="' . $id_sanitized . '_preview" type="text" class="at-font-select-preview" value="' . $meta['size'] . " " . esc_attr( $face_display ) . "/" . $meta['weight'] . '" readonly><a href="javascript:void(0);" class="button at-font-select" data-target="' . esc_attr( $id_sanitized ) . '">' . esc_html__( 'Select', 'offtheshelf' ) . '</a>';
		...
		public function show_field_color( $field, $meta ) {
			...
			if ( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
				...
				echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . esc_html__( 'Select a color', 'offtheshelf' ) . "'/>";
		...
		public function show_field_posts( $field, $meta ) {
			...
			if ( 'checkbox_list' == $options['type'] ) {
				...
				echo '<option value="0">' . esc_html__( 'None', 'offtheshelf' ) . '</option>';
				foreach ( $posts as $p ) {
    ...
    <div class="apc_ie_panel field">
      <div style="padding 10px;" class="apc_export"><h3>' . esc_html__( 'Export', 'offtheshelf' ) . '</h3>
        <p>' . offtheshelf_esc_html( __( 'Click the <code>Get Export Code</code> button below to retrieve the latest settings from the database. The export text can be copied and pasted to the import box of another installation, or to restore a previous state.', 'offtheshelf' ) ) . '</p>
        <div class="export_code">
          <label for="export_code">' . esc_html__( 'Export Code', 'offtheshelf' ) . '</label>
          <textarea id="export_code"></textarea>
          <a class="button-primary" id="apc_export_b" src="javascript:void(0);">' . esc_html__( 'Get Export Code', 'offtheshelf' ) . '</a> ' . $this->create_export_download_link() . '
          <div class="export_status" style="display: none;"><span>' . esc_html__( 'Loading...', 'offtheshelf' ) . '</span></div>
          <div class="export_results alert" style="display: none;"></div>
        </div>
      </div>
      <div style="padding 10px;" class="apc_import"><h3>' . esc_html__( 'Import', 'offtheshelf' ) . '</h3>
        <p>' . offtheshelf_esc_html( __( 'To import saved settings, paste the export output in to the <code>Import Code</code> box below and click <code>Import</code>.', 'offtheshelf' ) ) . '</p>
        <div class="import_code">
          <label for="import_code">' . esc_html__( 'Import Code', 'offtheshelf' ) . '</label>
          <textarea id="import_code"></textarea>
                  <input class="button-primary" type="button"  value="' . esc_html__( 'Import', 'offtheshelf' ) . '" id="apc_import_b" />
                  <input class="button-secondary" type="button"  value="' . esc_html__( 'Factory Defaults', 'offtheshelf' ) . '" id="apc_import_defaults" />
          <div class="import_status" style="display: none;"><span>' . esc_html__( 'Loading...', 'offtheshelf' ) . '</span></div>
    ...
    ';
		...
		public function export() {
			...
			if ( ! isset( $_GET['group'] ) ) {
				$re['err']   = esc_html__( 'error in ajax request! (1)', 'offtheshelf' );
			...
			if ( $options !== false ) {
				...
				$re['err'] = esc_html__( 'error in ajax request! (2)', 'offtheshelf' );
			}
		...
		public function defaults() {
			...
			$re['code'] = offtheshelf_file_read_contents( get_template_directory() . DIRECTORY_SEPARATOR . 'inc' . DIRECTORY_SEPARATOR . 'options.json' );
		...
		public function import() {
			...
			if ( ! isset( $_POST['imp'] ) ) {
				$re['err']   = esc_html__( 'error in ajax request! (3)', 'offtheshelf' );
			...
			if ( is_array( $import_code ) ) {
				...
				$re['success'] = esc_html__( 'Settings successfully imported. Make sure you ', 'offtheshelf' ) . '<input class="button-primary" type="button"  value="' . esc_html__( 'Refresh this page', 'offtheshelf' ) . '" id="apc_refresh_page_b" />';
			} else {
				$re['err'] = esc_html__( 'Could not import settings! (4)', 'offtheshelf' );
			}
			//update_nonce
			$re['nonce'] = wp_create_nonce( "apc_import" );
			echo json_encode( $re );
		...
		public function download_file( $content = null, $file_name = null ) {
			...
			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( '<p>' . esc_html__( 'You do not have sufficient permissions to edit templates for this site.', 'offtheshelf' ) . '</p>' );
			}

			if ( $content === null || $file_name === null ) {
				wp_die( '<p>' . esc_html__( 'Error Downloading file.', 'offtheshelf' ) . '</p>' );
			}
		...
		public function create_export_download_link( $echo = false ) {
			...
			if ( $echo === true ) {
				echo '<a href="' . esc_url($export_url) . '" target="_blank">' . esc_html__( 'Download', 'offtheshelf' ) . '</a>';
			} elseif ( $echo == 'url' ) {
				return $export_url;
			}

			return '<a class="button-primary" href="' . esc_url($export_url) . '" target="_blank">' . esc_html__( 'Download', 'offtheshelf' ) . '</a>';
		}
	 ...
	 */
		public function fonts_refresh() {
			...
			$re            = array();
			$re['nonce']   = wp_create_nonce( "web_fonts_refresh_nonce" );
			$re['success'] = esc_html__( 'The fonts database has been refreshed.', 'offtheshelf' );
			$re['status']  = (int) 200;
			echo json_encode( $re );
			die();
		}
			...
			foreach ( $field['validate'] as $type => $args ) {
				if ( method_exists( $this, 'is_' . $type ) ) {
					if ( call_user_func( array( $this, 'is_' . $type ), $meta, $args['param'] ) === false ) {
						...
						$this->errors[ $field['id'] ]['m'][]  = ( isset( $args['message'] ) ? $args['message'] : esc_html__( 'Not Valid ', 'offtheshelf' ) . $type );
						$ret                                  = false;
					}
				}
			}
		...
		public function displayErrors() {
			if ( $this->errors_flag ) {
				echo '<h4>' . esc_html__( 'Errors in saving changes', 'offtheshelf' ) . '</h4>';
		...
		public function getFieldErrors( $field_id ) {
			...
			return esc_html__( 'Unknown Error', 'offtheshelf' );
		}
