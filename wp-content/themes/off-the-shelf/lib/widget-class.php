	...
	class SR_Widget extends WP_Widget
	{
		...
		function create_field_posts( $key, $out = "" )
		{
			...
			if ($posts) {
				...
				$out .= '<option value="0"' . selected($key['value'], 0, false) . '>'. esc_html__( 'None', 'offtheshelf' ) . '</option>';
				foreach ($posts as $p) {
		...
		function create_field_categories( $key, $out = "" )
		{
			...
			if ($categories) {
				...
				if (! $key['multiple']) {
					...
					$out .= '<option value="0"' . selected($key['value'], 0, false) . '>'. esc_html__( 'All', 'offtheshelf' ) . '</option>';
		...
		function create_field_slider( $key, $out = "" )
		{
			...
			$out .= '<small class="description">'. esc_html__('Select control and user interaction options.', 'offtheshelf' ) . '</small><br/>';
			...
			if  (!in_array('controls', $key['hidden'] ) ) {
				...
				$out .=	'/> ';
				$out .=  ' ' . esc_html__('Display navigation arrows', 'offtheshelf') . '</label>';
				$out .= '<br/>';
			}
			...
			if  (!in_array('pips', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Display item navigation pips', 'offtheshelf') . '</label>';
			...
			if  (!in_array('pauseonhover', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Pause when the user hovers over the slider', 'offtheshelf') . '</label>';
			...
			if  (!in_array('pauseonaction', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Pause when the user interacts with control elements', 'offtheshelf') . '</label>';
			...
			if  (!in_array('randomize', $key['hidden'] ) ) {
				...
				$out .= checked( $meta , true, false );
				$out .= ' /> ';
				$out .=  ' ' . esc_html__('Randomize slide order', 'offtheshelf') . '</label>';
			...
			if  ( !in_array('transition', $key['hidden']) && !in_array('speed', $key['hidden']) ) {
				$out .= '<br/><small class="description">' . esc_html__('Select which transition to use and set the speed of the slideshow cycling in milliseconds.', 'offtheshelf' ) . '</small><br/>';
			}
			elseif ( in_array('transition', $key['hidden'])  ) {
				$out .= '<br/><small class="description">' . esc_html__('Set the speed of the slideshow cycling in milliseconds.', 'offtheshelf' ) . '</small><br/>';
			} else {
				$out .= '<br/><small class="description">'. esc_html__( 'Select which transition to use.', 'offtheshelf' ) . '</small><br/>';
			}
			...
			if  (!in_array('transition', $key['hidden'] ) ) {
				...
				$options = array(
						array( 'value' => 'slide', 'name' => esc_html__('Slide', 'offtheshelf') ),
						array( 'value' => 'fade', 'name'  => esc_html__('Fade', 'offtheshelf') ),
				);
		...
		function create_field_gallery( $key, $out = "" )
		{
			...
			$out .= '<input type="button" class="button gallery-picker-select" data-target="#' . esc_attr( $key['_id'] ) . '" id="'. esc_attr( $key['_id'] ) . '_ids' .'" value="'.esc_html__('Select Items', 'offtheshelf').'">';
		...
		function create_field_fontawesome( $key, $out = "" )
		{
			...
			$out .= '<input type="button" class="button fontawesome-picker" data-target="#' . esc_attr( $key['_id'] ) . '" id="'. esc_attr( $key['_id'] ) . '_picker' .'" value="'.esc_html__('Select Icon', 'offtheshelf').'">';
		...
		function create_field_image( $key, $out = "" )
		{
			...
			if ( isset ($preview_url ) && $preview_url ) {
				$out .= '<div class="ots-widget-preview"><img class="ots-widget-preview-image" src="' . esc_url( $preview_url ) . '"></div>';
			} else {
				$out .= '<div class="ots-widget-preview"><img class="ots-widget-preview-image empty" src="' . get_template_directory_uri() . '/lib/admin/images/transparent.png"></div>';
			}
			...
			$out .= '<input type="button" class="button select-img select-img-widget" data-target="' . esc_attr( $key['_id'] ) . '" value="' . esc_html__('Select Image', 'offtheshelf') . '" />';
			$out .= '<input type="button" class="button select-img clear-img-widget" data-target="' . esc_attr( $key['_id'] ) . '" data-preview="' . get_template_directory_uri() . '/lib/admin/images/transparent.png" value="' . esc_html__('Clear', 'offtheshelf') . '" />';
