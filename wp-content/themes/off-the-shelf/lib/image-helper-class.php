	...
	private function resize_image($orig_size, $dest_size, $dest_file_name) {
		...
		if (is_wp_error($image)) {
			if (is_admin()) {
				...
				echo '<p><strong>' . esc_html__('ERROR', 'offtheshelf') .'</strong> ' . $image->get_error_message() . ' ' . esc_html__('Check file permissions.', 'offtheshelf') . '</p>';
				echo '<button class="toggle">' . esc_html__('Show Details', 'offtheshelf') . '</button>';
				echo '<div class="message" style="display: none;"><br />' . esc_html__('Slide ID: ', 'offtheshelf') . $this->id . '<pre>';
				var_dump($image);
				echo '</pre></div>';
				echo '</div>';
			}
